...
/*
 *  NITINDYT CLIENT LAUNCHER - Minecraft Launcher
...
PrismUpdaterApp::PrismUpdaterApp(int& argc, char** argv) : QApplication(argc, argv)
{
    ...
    parser.setApplicationDescription(QObject::tr("An auto-updater for NITINDYT CLIENT LAUNCHER"));
    ...
    m_updateLogPath = FS::PathCombine(m_dataPath, "logs", "nitindyt_client_update.log");
    ...
    auto marker_file_path = QDir(m_rootPath).absoluteFilePath(".nitindyt_client_updater_unpack.marker");
...
void PrismUpdaterApp::run()
{
    ...
    if (m_isFlatpak) {
        showFatalErrorMessage(tr("Updating flatpack not supported"), tr("Actions outside of checking if an update is available are not "
                                                                        "supported when running the flatpak version of NITINDYT CLIENT LAUNCHER."));
        return;
    }
    if (m_isAppimage) {
        bool result = true;
...
void PrismUpdaterApp::moveAndFinishUpdate(QDir target)
{
    ...
    if (error) {
        ...
        auto fail_marker = FS::PathCombine(m_dataPath, ".nitindyt_client_update.fail");
        FS::copy(m_updateLogPath, fail_marker).overwrite(true)();
    } else {
        logUpdate(tr("Update succeed."));
        auto success_marker = FS::PathCombine(m_dataPath, ".nitindyt_client_update.success");
        FS::copy(m_updateLogPath, success_marker).overwrite(true)();
    }
    auto update_lock_path = FS::PathCombine(m_dataPath, ".nitindyt_client_update.lock");
...
void PrismUpdaterApp::performInstall(QFileInfo file)
{
    ...
    if (update_lock.exists()) {
        ...
        auto infoMsg =
            tr("This installation has a update lock file present at: %1\n"
               ...
               "proceeding.\n"
               "Check the NITINDYT CLIENT LAUNCHER updater log at: \n"
               "%7\n"
    ...
    auto changelog_path = FS::PathCombine(m_dataPath, ".nitindyt_client_update.changelog");
...
void PrismUpdaterApp::unpackAndInstall(QFileInfo archive)
{
    ...
    if (auto loc = unpackArchive(archive)) {
        auto marker_file_path = loc.value().absoluteFilePath(".nitindyt_client_updater_unpack.marker");
...
void PrismUpdaterApp::backupAppDir()
{
    ...
    if (file_list.isEmpty()) {
        ...
        if (BuildConfig.BUILD_ARTIFACT.toLower().contains("linux")) {
            file_list.append({ "nitindyt-client", "bin", "share", "lib" });
        } else {  // windows by process of elimination
            file_list.append({
                "jars",
                "nitindyt-client.exe",
                "nitindyt-client_filelink.exe",
                "nitindyt-client_updater.exe",
                "qtlogging.ini",
                "imageformats",
                "iconengines",
                "platforms",
    ...
    auto backup_marker_path = FS::PathCombine(m_dataPath, ".nitindyt_client_update_backup_path.txt");
...
{
    auto temp_extract_path = FS::PathCombine(m_dataPath, "nitindyt_client_update_release");
