...
#include "ui/pages/global/OptimizationPage.h"
...
Application::Application(int& argc, char** argv) : QApplication(argc, argv)
{
    ...
    /*
     * Establish the mechanism for communication with an already running NITINDYT CLIENT LAUNCHER that uses the same data path.
    ...
    // Initialize application settings
    {
        ...
        // Init page provider
        {
            ...
            m_globalSettingsProvider->addPage<OptimizationPage>();
    ...
    // check update locks
    {
        auto update_log_path = FS::PathCombine(m_dataPath, "logs", "nitindyt_client_update.log");

        auto update_lock = QFileInfo(FS::PathCombine(m_dataPath, ".nitindyt_client_update.lock"));
...
"Check the NITINDYT CLIENT LAUNCHER updater log at: \n"
        ...
        auto update_fail_marker = QFileInfo(FS::PathCombine(m_dataPath, ".nitindyt_client_update.fail"));
...
"Check the NITINDYT CLIENT LAUNCHER updater log at: \n"
        ...
        auto update_success_marker = QFileInfo(FS::PathCombine(m_dataPath, ".nitindyt_client_update.success"));
...
bool Application::createSetupWizard()
{
    ...
    if (wizardRequired) {
        ...
        if (!validWidgets) {
            settings()->set("ApplicationTheme", QStringLiteral("nitindyt-dark"));
        }
